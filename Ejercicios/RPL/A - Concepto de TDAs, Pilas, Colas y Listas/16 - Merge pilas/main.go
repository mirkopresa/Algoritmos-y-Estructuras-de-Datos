...
func ApilarSinRepetir(pila Pila[int], desapilado int) {
...
func MergePilas(pila1, pila2 Pila[int]) []int {
	...
	for !pila1.EstaVacia() && !pila2.EstaVacia() {
		...
		tope2 := pila2.VerTope()
		if tope1 > tope2 {
			ApilarSinRepetir(pilaAux, pila1.Desapilar())
		} else {
			ApilarSinRepetir(pilaAux, pila2.Desapilar())
		}
	}
	for !pila1.EstaVacia() {
		ApilarSinRepetir(pilaAux, pila1.Desapilar())
	...
	for !pilaAux.EstaVacia() {
		resultado = append(resultado, pilaAux.Desapilar())
	}
	return resultado
}
